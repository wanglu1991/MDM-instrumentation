...
def generate_PC_miss_info(SM_numbers):
   ...
   #     memory_pc_info.write(str(k)+','+str(len(total_access_pc[k]))+','+str(len(L1_miss_pc[k]))+'\n')
        L1_miss_rate=float(len(L1_miss_pc[k]))/float(len(total_access_pc[k]))
        if k in L2_miss_pc:
            L2_miss_rate=float(len(L2_miss_pc[k]))/float(len(L1_miss_pc[k]))
            latency=(L2_miss_rate*250+(1-L2_miss_rate)*530)*L1_miss_rate+85
        else:
            latency=85
        print(latency)
