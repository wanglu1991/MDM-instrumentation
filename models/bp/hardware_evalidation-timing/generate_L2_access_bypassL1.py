...
import os
import sys
def generate_L1_hit_info(SM_numbers):
...
def generate_L2_hit_info():
    cmd='./cache-simulator L2_cache L2_trace.txt output_L2 output_L2_access.txt'
    os.system(cmd)
...
def main(kernel_id):
    ...
    SM_numbers=28
 #   generate_L1_hit_info(SM_numbers)
    generate_L2(SM_numbers)
    generate_L2_hit_info()
...
def generate_PC_miss_info(SM_numbers):
   ...
   #     miss_rate=float(len(L1_miss_pc[k]))/float(len(total_access_pc[k]))
        if k in L2_miss_pc:
            ...
            latency=L2_miss_rate*220+(1-L2_miss_rate)*120
            print(latency)
...
def generate_L2(SM_numbers):
    ...
    for i in range(0,SM_numbers):
        SM_miss_list=[]
        memory_trace=open('SM_trace_'+str(i)+'.txt','r')
        for line in memory_trace:
            access_info=[]
