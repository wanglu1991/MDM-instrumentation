...
class interval(object):
    def __init__(self):
        ...
        self.dram_bandwidth=320.0
        self.noc_bandwidth=0
    ...
    def reset_NoC(self,NoC_size):
        self.noc_bandwidth=NoC_size
        self.queuing_delay_mshr_optimal=[]
        self.queuing_delay_mshr_optimal_new=[]
        self.queuing_delay_dram=[]
        self.queuing_delay_noc=[]
    ...
    def calculate_avg_stall_cycles(self):
        ...
        self.avg_stall_cycles=250*(1-miss_rate)+530*miss_rate
    ...
    def RR_regioninsts(self):
        ...
        while(i<len(self.region_instructions[self.warp_index])-1):
            ...
            self.nonoverlap_insts+=(self.warps_per_scheduler-1)*(current_inst-1)
            if (self.region_instructions[self.warp_index][i+1]<(self.warps_per_scheduler-1)/self.max_issue):
                self.nonoverlap_insts+=(self.warps_per_scheduler-1)/self.max_issue-self.region_instructions[self.warp_index][i+1]
            i+=2
        issue_ration=float(self.accum_inst)/(float(self.accum_inst)/self.max_issue+float(self.accum_stall))*self.warps_per_scheduler
...
#def multi_RR(nonoverlap_insts,warp_numbers):
    def IPC_full(self):
        ...
        if(multi_ipc>self.max_issue):
            ipc=self.max_issue
        print(multi_ipc)
        print("total_instruction_counts")
        ...
        print(GPUMech_MSHR_total)
        print(DRAM_delay_total)
        print(GPUMech_cycles)
        GPUMech_ipc=float(self.accum_inst)*self.warp_numbers/float(GPUMech_cycles)
        print(GPUMech_ipc)
        result=open('GPUMech_'+str(self.noc_bandwidth),'a')
        result.write(str(GPUMech_ipc)+str(',')+str(self.total_instruction_counts)+'\n')
        NoC_delay_total=0
        ...
        new_MSHR_result=open('result_delay_model+ceiling_MSHR_'+str(self.noc_bandwidth),'a')
    ...
    def Get_NoC_info(self):
        ...
        for i in range(0,self.activeCores):
    ...
    def warp_interval_profile(self,kernel_id):
           ...
           interval_inst={}
           interval_stalls={}
           dependence_trace=open('../instruction_trace_'+str(kernel_id)+'.txt','r')
      ...
      # output_trace=open('single_warp_info.txt','w')
           for line in dependence_trace:
                warp_id=int(line.split(',')[0])
                if(reg_write.get(warp_id,0)==0):
                    reg_write[warp_id]=[]
                    for regs_number in range(0,1000):
                        reg_write[warp_id].append(0)
                    self.region_instructions[warp_id]=[]
                    interval_inst[warp_id]=0
                    interval_stalls[warp_id]=0
                ...
                if(pc in pc_list):
                   ...
                   # print(pc)
                source_reg_1=int(line.split(',')[4])
                source_reg_2=int(line.split(',')[5])
                source_reg_3=int(line.split(',')[6])
                destination_reg = int(line.split(',')[3])
                issue_cycle=warp_max_issue_cycle[warp_id]+1
                if((source_reg_1<1000)and(source_reg_1>0)):
                    if(reg_write[warp_id][source_reg_1]>issue_cycle):
                        issue_cycle=reg_write[warp_id][source_reg_1]
                if((source_reg_2<1000)and(source_reg_2>0)):
                    if(reg_write[warp_id][source_reg_2]>issue_cycle):
                        issue_cycle=reg_write[warp_id][source_reg_2]
                if((source_reg_3<1000)and(source_reg_3>0)):
                    if(reg_write[warp_id][source_reg_3]>issue_cycle):
                        issue_cycle=reg_write[warp_id][source_reg_3]
                if((destination_reg<1000)and(destination_reg>0)):
                    if((issue_cycle+latency)>(reg_write[warp_id][destination_reg])):
                        reg_write[warp_id][destination_reg]=issue_cycle+latency+1
                #print(reg_write[55][5])
                #print(warp_max_issue_cycle[55])
                if((issue_cycle-warp_max_issue_cycle[warp_id])<10):
                    interval_inst[warp_id]+=1
                    interval_stalls[warp_id]+=issue_cycle-warp_max_issue_cycle[warp_id]
                else:
                    stall_cycles=issue_cycle-warp_max_issue_cycle[warp_id]
                    self.region_instructions[warp_id].append(interval_inst[warp_id])
                    self.region_instructions[warp_id].append(stall_cycles+interval_stalls[warp_id])
                    interval_inst[warp_id]=1
                    interval_stalls[warp_id]=0
           ...
           #print(self.region_instructions)
...
def main(kernel_id):
   ...
   # intervalInstance.contention_DRAM_modeling()
    #intervalInstance.Get_NoC_info()
    NoC_config=[525.0,1050.0,1700.0,2100.0,4200.0,8400.0]
    for i in range(0,6):
        NoC_size=NoC_config[i]
        intervalInstance.reset_NoC(NoC_size)
        start=timer()
       ...
       # print('dram_modeling')
        ...
        duration=end-start
        timing=open('./timing_overhead.txt','a')
        timing.write('1,'+str(duration)+'\n');
